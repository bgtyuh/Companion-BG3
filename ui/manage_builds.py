...
class ManageBuildsWidget(QWidget):
    ...
    def load_builds(self):
        ...
        for build in builds:
            item = QListWidgetItem(f"{build[1]} - {build[2]} - {build[3]} - {build[4]} - {build[5]}")
    ...
    def modify_build(self, build_id):
        ...
        # Appeler une méthode ou afficher une nouvelle interface pour modifier le build
        pass
    ...
    def get_build_data(self, build_id):
        ...
        cursor.execute("SELECT race_name, class_name, weapon_name, armor_name, footwear_name FROM builds WHERE id = ?", (build_id,))
